// UPDATED: app/embed/page.js
// Better error handling and production support
...
const EmbedPage = () => {
  ...
  useEffect(() => {
    if (!token) {
      setConfigError('Missing chatbot token. Please check your embed code.')
      return
    }

    const apiUrl = process.env.NEXT_PUBLIC_APP_URL || window.location.origin

    fetch(`${apiUrl}/api/widget/config?token=${token}`)
      .then((r) => {
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          setConfigError(data.error || 'Failed to load widget configuration.')
        }
      })
      .catch((err) => {
        console.error('Config load error:', err)
        setConfigError(`Network error: ${err.message}. Please check your connection and chatbot ID.`)
      })
  }, [token])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/chat/test`, {
        method: 'POST',
  ...
  if (configError) {
    return (
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100vh', background:'#09090b', color:'#71717a', fontSize:'13px', fontFamily:'sans-serif', padding:'20px', textAlign:'center' }}>
        <div style={{ fontSize:'32px', marginBottom:'12px' }}>⚠️</div>
        <div style={{ fontWeight:600, marginBottom:'8px', color:'#f5f5f5' }}>Chatbot Configuration Error</div>
        <div style={{ maxWidth:'400px' }}>{configError}</div>
        {token && (
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px', display:'flex', flexDirection:'column', gap:'12px' }}>
        ...
        {loading && (
          <div style={{ display:'flex', alignItems:'flex-end', gap:'8px' }}>
            <div style={{ width:'26px', height:'26px', borderRadius:'8px', background:`${accentColor}22`, border:`1px solid ${accentColor}44`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z" fill={accentColor}/>
              </svg>
            </div>
